...
try {
    ...
    $backendCmd = "cd 'd:\OMS Chat Bot\backend'; Write-Host '🔥 BACKEND SERVER' -ForegroundColor Cyan; & 'D:\OMS Chat Bot\.venv\Scripts\python.exe' -m uvicorn app.main:app --host 127.0.0.1 --port 8000"
...
try {
    ...
    $frontendCmd = "cd 'd:\OMS Chat Bot\frontend'; Write-Host '🌟 FRONTEND SERVER' -ForegroundColor Cyan; npm run dev"
    ...
    try {
        ...
        Write-Host "   Check the frontend window for progress" -ForegroundColor Yellow
    }
}
...
try {
    ...
    Write-Host "⚠️  Could not fetch system status (backend may still be starting)" -ForegroundColor Yellow
    Write-Host ""
}
